...
export const usersRoutes = (controller: UsersController, authService: AuthService) => {
  ...
  router.post('/register', authRateLimit, validateBody(schemas.registerUser), controller.register);
  router.post('/login', authRateLimit, validateBody(schemas.loginUser), controller.login);
